...
# Refuses a task claimed by someone else, and never merges a money/pii task that has no
...
if [ "$(task_field "$t" money)" = "true" ] || [ "$(task_field "$t" pii)" = "true" ]; then
  n="$(gh pr view "$PRNUM" --json comments -q '[.comments[] | select(.body | test("review"; "i"))] | length' 2>/dev/null || echo 0)"
  [ "$n" -gt 0 ] || die "$ID is money/pii and PR #$PRNUM has no review comment: run 'skein review $ID' first"
fi
